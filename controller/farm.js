...
class newhouseController{
	...
	async add(farm){
		try{
			let response = await farmSchema.create(farm);
	...
	async fetchdata(id){
		try{
			let response = await farmSchema.find({'_id':id});
			return response;
	...
	async delete(id){
		try{
			let response = await farmSchema.deleteOne({_id: id});
			return {
				status: "success",
				response: response
			};
	...
	async update(id, body) {
        ...
        try {
            let response = await farmSchema.update({_id: id}, body);
