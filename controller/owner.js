...
class OwnerController {
    ...
    async register(newGender){
        try{
            await ownerSchema.create(newGender);
   return {
                status: 'success',
                msg: 'User created'
            }
    ...
    async login(responce){
        ...
        try{
            let user = await ownerSchema.findOne({
                username: username,
                password: password,
            });
	...
	async fetchdata(id){
		try{
			let response = await ownerSchema.find({_id: id});
			return response;
	...
	async delete(id){
		try{
			let response = await ownerSchema.deleteOne({_id: id});
			return {
				status: "success",
				response: response
			};
